...
\begin{tikzpicture}[
  ...
  % Fan-out connector: one stem down from the Orchestrator, a horizontal bus,
  % then a drop to each worker. A bare "(orch.south -| extractor)" endpoint
  % only touches the Orchestrator box when extractor's x falls inside its
  % width -- it doesn't here, so that line floated with no visible source.
  \coordinate (bus) at ($(orch.south)+(0,-0.35)$);
  \draw[gray!60, dashed, line width=0.8pt] (orch.south) -- (bus);
  \draw[gray!60, dashed, line width=0.8pt] (extractor.north |- bus) -- (loader.north |- bus);
  \draw[darr] (extractor.north |- bus) -- (extractor.north);
  \draw[darr] (transformer.north |- bus) -- (transformer.north);
  \draw[darr] (loader.north |- bus) -- (loader.north);
\end{tikzpicture}%
}
\end{center}
