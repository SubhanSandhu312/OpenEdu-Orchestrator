...
\node[draw, thick, rounded corners, fill=NavyBlue!5, align=left, text width=12.5cm, inner sep=14pt] (box) {
    ...
    \textbf{Result:} 4 cooperating agents + 1 optional Validation agent, 1 LangGraph state machine serving 3 sync cycles, 3 pluggable source adapters, 2 interchangeable targets (mock and real Odoo), a design-time LLM-assisted mapping-authoring tool, a read-only reconciliation/drift-audit command, and an \textbf{84-test pytest suite} running green in CI on Python 3.11 and 3.12.
};
\end{tikzpicture}
\end{center}
\vfill
...
\begin{itemize}[leftmargin=1.4em]
    ...
    \item is verified by an \textbf{84-test pytest suite} (up from 57) that remains fully
...
Tests & 57, run manually & 84, run in CI on Python 3.11 and 3.12 \\
\bottomrule
\end{tabular}
...
\texttt{pytest} & Test suite & 84 tests with isolated per-test databases via \ttu{tmp_path}. \\
\bottomrule
\end{tabular}
...
\begin{axis}[
    ...
    symbolic x coords={sync-store,source-general,mapping-author,pieas-source,orch-classify,
                        validator,transformer,retry,oc-client,loader,extractor,
                        deletion-cycle,change-sync,bulk-migration,logging,end-to-end},
    xtick=data,
...
\addplot[fill=AgentBlue, draw=NavyBlue] coordinates {
    (sync-store,9) (source-general,9) (mapping-author,8) (pieas-source,6) (orch-classify,6)
    (validator,5) (transformer,5) (retry,5) (oc-client,5) (loader,5) (extractor,5)
    (deletion-cycle,4) (change-sync,4) (bulk-migration,4) (logging,3) (end-to-end,1)
};
\end{axis}
\end{tikzpicture}
\caption{Distribution of the 84-test suite across 16 test modules. The four rightmost-growing
areas (\texttt{source-general}, \texttt{mapping-author}, \texttt{retry}, \texttt{logging}) are
entirely new in Revision~2.}
\label{fig:testchart}
\end{figure}
...
The following are deliberately \emph{not} implemented, because they depend on decisions that are
...
src/openedu_orchestrator/
    ...
    real_target_reference_data.py  # shared reference-data resolution (department -> department_id)
...
tests/                        # pytest suite (84 tests, 16 modules)
